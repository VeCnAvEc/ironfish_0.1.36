...
export class MiningPool {
  ...
  async informationAboutTheBlock(condition: any, time: string | number) {
    if ( condition.hashedHeader && condition.height) {
        await this.shares.informationAboutTheBlock(condition, time)
        // console.log('Successfully')
    }}
    async userHashRate(publicAddress: string | null | undefined): Promise<number> {
      const userRate = await this.shares.userRate(publicAddress)
    ...
    async hashRateForGraphics(hashRateFifteenMinutes: any) {
      this.shares.hashRateForGraphics(hashRateFifteenMinutes)
    }
 
    async gethashRateFifteenMinutes() {  
     return await this.shares.gethashRateFifteenMinutes()
    }
  ...
  async createUserFields(publickey: string | null, timestamp: any, online: boolean, lastMining: any): Promise<void> {
    await this.shares.createUserFields(publickey, timestamp, online, lastMining)
  }
 
  async setOfflineUser(publicAddress: string, time: number) {
    await this.shares.setOfflineUser(publicAddress, time)
  }

  async setOnlineUser(publicAddress: string) {
    await this.shares.setOnlineUser(publicAddress)
  }
 
  async userHashForGraphics(date: Array<any>) {
    return await this.shares.userHashForGraphics(date)
  }

  async setAllUsersStatusOfline() {
    await this.shares.setAllUsersStatusOfline()
  }

  async getUserHashRateGraphics(publicAddress: string) {
    return this.shares.getUserHashRateGraphics(publicAddress)
  }

  async removeOldRecordUserEightHours() {
    await this.shares.removeOldRecordUserEightHours()
  }
 
  async removeOldRecordEightHours() {
   await this.shares.removeOldRecordEightHours()
  }

   async findUserByPublicAddress(publicAddress: string) {
     return await this.shares.findUserByPublicAddress(publicAddress)
   }

   async getAmountUser (publicAddress: string) {
       return await this.shares.getAmountUser(publicAddress)
   } 
 
   async getTheTotalPayoutOfThePool() {
     return await this.shares.getTheTotalPayoutOfThePool()
   }

   async getTransaction() {
    return await this.shares.getTransaction()
  }
  
  async getTheUserPayout(publicAddress: string) {
    return await this.shares.getTheUserPayout(publicAddress)
  }

  async totalUsers() {
    return await this.shares.totalUsers()
  }

  async getAllUsers() {
    return await this.shares.getAllUsers()
  }

  async getAllBlock() {
    return await this.shares.getAllBlock()
  }

  async getPaidCoins() {
    return await this.shares.getPaidCoins()
  }

  async getAllShares() {
    return await this.shares.getAllShares()
  }

  async getAllPayout() {
    return await this.shares.getAllPayout()
  }
